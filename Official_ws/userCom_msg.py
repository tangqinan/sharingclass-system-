...
def userCom_msg_data(request,email):
    try:
        ...
        new_count = models.UserTable.objects.filter(email=email)[0].read_count_u
...
def userCom_msg_data_update(request):
    try:
        if request.method == "GET":
            ...
            count = query_set.count()
            models.UserTable.objects.filter(email=userId).update(read_count_u=count)
